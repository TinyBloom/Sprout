...
def segment_mapping(scores):
    ...
    very_low_mask = (scores >= 1) & (scores < 1)
...
def adjust_health_score(scores, min_boost=20, mid_target=70, power=0.7):
    ...
    if mid_target != 50:
        ...
        if adjustment_factor > 0:
            ...
            boosted[below_mid] = target_for_50 + (boosted[below_mid] - target_for_50) * (1 + adjustment_factor)
            # Compress scores above the middle value
            factor_high = (100 - mid_target) / (100 - target_for_50)
            boosted[above_mid] = mid_target + (boosted[above_mid] - mid_target) * factor_high
