...
def training_temperature_model(job):
    ...
    X, y = create_sliding_window(
        data, window_size, "temperature", ["hour", "dayofweek", "is_weekend"]
    )
    ...
    future_timestamps = pd.date_range(
        start=data["time"].iloc[-1] + pd.Timedelta(seconds=60), periods=4320, freq="1T"
    )

    future_df = pd.DataFrame(
        {
            "Timestamp": future_timestamps,
            "Predicted_Temperature": filter_data(
                job.params, future_predictions_original
            ).flatten(),
        }
    )
