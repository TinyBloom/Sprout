...
# 1. 加载数据
file_path = 'kuka_axis_run_info_1345904_A6.csv'  # 替换为你的数据路径
...
isolation_forest = IsolationForest(
    ...
    contamination=0.0001,        # 假定异常点的比例为 1%
    random_state=42
)
...
min_max_scaler.fit(robust_scaler.transform(scores.reshape(-1, 1)))
health_score_percentage = min_max_scaler.transform(scaled_scores.reshape(-1, 1)).flatten()


data["unadjust_health_score_percentage"] = health_score_percentage
